...
export function smsg(conn, m, hasParent) {
    ...
    // Evita erro caso 'proto' não esteja no escopo global e usa a biblioteca importada
    let M = typeof proto !== 'undefined' ? proto.WebMessageInfo : m.constructor
    ...
    if (m?.message) {
        if (m?.mtype === 'protocolMessage' && m?.msg?.key) {
            ...
            protocolMessageKey.fromMe = conn.decodeJid(protocolMessageKey.participant) === conn.decodeJid(conn.user?.id || conn.user?.jid)
            if (!protocolMessageKey.fromMe && protocolMessageKey.remoteJid === conn.decodeJid(conn.user?.id || conn.user?.jid)) protocolMessageKey.remoteJid = m.sender
        }
        
        // Verificação segura de m.quoted antes de acessar suas propriedades
        if (m?.quoted) {
