...
getName: {
            ...
            value(jid = '', withoutContact = false) {
                ...
                try {
                    ...
                    jid = typeof jid === 'string' ? jid : (jid?.jid || jid?.id || '')
                }

                if (!jid) return 'Membro'

                let cleanNum = jid.replace(/[^0-9]/g, '')
                if (!cleanNum) return 'Membro'

                let fullJid = cleanNum + '@s.whatsapp.net'

                // 1. WhatsApp Oficial / Próprio Bot
                if (jid === '0@s.whatsapp.net') return 'WhatsApp'
                if (conn.user && areJidsSameUser(jid, conn.user.id)) return conn.user.name || conn.user.vname || 'Bot'

                // 2. Se for Grupo
                if (jid.endsWith('@g.us')) {
                    let gChat = conn.chats[jid] || {}
                    return gChat.name || gChat.subject || `@${cleanNum}`
                }

                // 3. Busca no banco de dados global (várias chaves possíveis)
                let userDB = global.db?.data?.users?.[fullJid] 
                    || global.db?.data?.users?.[jid] 
                    || global.db?.data?.users?.[cleanNum]

                if (userDB && (userDB.name || userDB.pushName)) {
                    return userDB.name || userDB.pushName
                }

                // 4. Busca no cache de conversas em memória do Baileys
                let v = conn.chats[fullJid] || conn.chats[jid] || {}
                let chatName = (withoutContact ? '' : v.name) || v.subject || v.vname || v.notify || v.verifiedName
                if (chatName) return chatName

                // 5. Fallback final com a tag do número (evita "undefined" em 100% dos casos)
                return `@${cleanNum}`
            },
            enumerable: true,
            writable: true
...
export function smsg(conn, m, hasParent) {
    ...
    if (m.key) {
        m.id = m.key.id
        m.isBaileys = m.id && (m.id.startsWith('BAE5') || m.id.startsWith('3EB0') || m.id.length === 16 || m.id.length === 22)
    ...
    if (m.message) {
        ...
        // Extração segura do corpo do texto
        ...
        // Tratamento seguro de Quoted (Mensagem Citada)
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {
                ...
                m.quoted.isBaileys = m.quoted.id ? (m.quoted.id.startsWith('BAE5') || m.quoted.id.startsWith('3EB0') || m.quoted.id.length === 16 || m.quoted.id.length === 22) : false
                m.quoted.sender = conn.decodeJid(m.msg.contextInfo.participant)
                m.quoted.fromMe = m.quoted.sender === conn.decodeJid(conn.user?.id)
                m.quoted.mentionedJid = m.msg.contextInfo?.mentionedJid || []

                // Proteção para m.quoted.text evitar o erro TypeError
                Object.defineProperty(m.quoted, 'text', {
                    get() {
                        return this.caption || this.conversation || this.contentText || this.selectedDisplayText || this.title || this._text || ''
                    },
                    set(val) {
                        this._text = val
                    },
                ...
                let isMedia = ['imageMessage', 'videoMessage', 'stickerMessage', 'audioMessage', 'documentMessage'].includes(m.quoted.mtype)
                if (isMedia) {
