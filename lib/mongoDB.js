...
export class mongoDB {
  ...
  /** Escreve/Atualiza os dados no banco de dados */
  write(data) {
...
/** Versão avançada do MongoDB que organiza os dados em múltiplas coleções */
export const mongoDBV2 = class MongoDBV2 {
  ...
  write(data) {
    return new Promise(async (resolve, reject) => {
      ...
      this.list.findById(this.lists._id, (err, doc) => {
        if (err) return reject(err)
        doc.data = listDoc
        return doc.save((err) => {
          if (err) return reject(err)
