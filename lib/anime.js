...
// Obtém scripts da página para extrair links ocultos
async function getScript(url) {
  try {
    ...
    console.error('Erro ao obter script:', error);
  }
}

// Extrai links do Pixeldrain (Legendado/Dublado)
function get(texto) {
  const regex = /"PDrain",url:"(https:\/\/pixeldrain\.com\/u\/[^"?"]+)/g;
  let match;
  const linkss = { sub: null, dub: null };
  const index = texto.indexOf('downloads:{');
  if (index === -1) return linkss;
...
async function download(url) {
  try {
    ...
    return { error: 'Falha ao buscar ou processar a página', details: err.message };
  }
}

// Obtém detalhes completos do anime
async function detail(url) {
  ...
  try {
    ...
    return {
      title, altTitle, description, rating, votes,
      cover, backdrop, genres, episodes, total: episodes.length
    };
  } catch (err) {
