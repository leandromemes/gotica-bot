...
const handler = async (m, {conn, command, usedPrefix}) => {
    // Verificação correta baseada na sua database (nsfw) 💫
    if (m.isGroup && !global.db.data.chats[m.chat].nsfw) {
        return m.reply('*🚩 O modo NSFW está desativado neste grupo! Peça a um ADM para ligar.* 🖤');
    }

    // Mensagem de envio estética ⭐
    await conn.reply(m.chat, `*💋 Preparando o envio de: ${command}...* ✨`, m, {
        contextInfo: { 
            externalAdReply: { 
                mediaUrl: null, 
                mediaType: 1, 
                showAdAttribution: true,
                title: 'Gótica Bot - Área +18🌙',
                body: 'By Gotica bot',
                previewType: 0, 
    ...
    switch (command) {
        case 'pack':
            ...
            await conn.sendMessage(m.chat, {image: {url: url}, caption: `*🥵 Aqui está seu Pack 💋*`}, {quoted: m});
            break;

        case 'pack2':
            const url2 = global.packgirl[Math.floor(Math.random() * global.packgirl.length)];
            await conn.sendMessage(m.chat, {image: {url: url2}, caption: `*🥵 Pack Feminino 💫*`}, {quoted: m});
            break;

        case 'pack3':
            const url3 = global.packmen[Math.floor(Math.random() * global.packmen.length)];
            await conn.sendMessage(m.chat, {image: {url: url3}, caption: `*🥵 Pack Masculino 🖤*`}, {quoted: m});
            break;

        case 'videoxxx':
        case 'vídeoxxx':
            const url4 = global.videosxxxc[Math.floor(Math.random() * global.videosxxxc.length)];
            await conn.sendMessage(m.chat, {video: {url: url4}, caption: `*🔥 Aproveite o vídeo... 🔞*`}, {quoted: m});
            break;
        ...
        case 'pornolesbivid':
            ...
            await conn.sendMessage(m.chat, {video: {url: url5}, caption: `*🔥 Conteúdo Lésbico 💋*`}, {quoted: m});
            break;
    }
};

handler.help = ['pack', 'pack2', 'videoxxx'];
handler.tags = ['adult'];
...
handler.group = true;
// Trava de registro removida 💋

export default handler;

// Listas de Mídia (Globais) ⭐
global.pack = [
